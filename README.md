# Shoppers Serenity Framework

Serenity Cucumber BDD test automation for **login** on the Shoppers ecommerce application.

**Documentation:** [docs/README.md](docs/README.md) · [docs/PROJECT_STRUCTURE.md](docs/PROJECT_STRUCTURE.md)

## Run locally

**PowerShell:** `.\run_login_positive.ps1`

**CMD:** `run_login_positive.bat`

Do not run `mvn ... -Dcucumber.filter.tags=...` in PowerShell without `--%` or quotes — use `-Dtags=` instead (see `docs/README.md`).
